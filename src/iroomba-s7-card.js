...
class IRoombaS7Card extends HTMLElement {
  ...
  render() {
    ...
    const binText=!binPresent?"Beholderen er ikke monteret":binFull?"Beholderen er fuld og bør tømmes":"Beholderen er monteret og ikke fuld";
    const binStatus=`<span class="serviceIcon binIcon ${!binPresent?"warn":binFull?"overdue":""}" title="${esc(binText)}" aria-label="${esc(binText)}" role="img"><ha-icon icon="${!binPresent?"mdi:delete-off":binFull?"mdi:delete-alert":"mdi:delete-outline"}"></ha-icon></span>`;
    const maintenance=[binStatus,this._maintenance("filter","Filter","mdi:air-filter"),this._maintenance("brush","Børster","mdi:brush-variant")].join("");
