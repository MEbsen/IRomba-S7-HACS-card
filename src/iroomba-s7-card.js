// Development builds are published automatically from this branch. Maintenance reset icons remain on the floor overlay.
const CARD_VERSION = "0.2.0-dev.8";
...
class IRoombaS7Card extends HTMLElement {
  ...
  _maintenance(part,label,icon){
    ...
    if(!saved||!Number.isFinite(now)){const text=saved?`${label}: mangler statistik for den valgte enhed`:`${label}: klik for at nulstille efter næste skift`;return `<button class="serviceIcon muted" data-reset="${part}" title="${esc(text)}" aria-label="${esc(text)}"><ha-icon icon="${icon}"></ha-icon></button>`;}
    ...
    const text=overdue?`${label}: skift nu · ${Math.abs(remaining)} ${units} over intervallet`:`${label}: ${remaining} ${units} tilbage`;
    return `<button class="serviceIcon ${overdue?"overdue":warn?"warn":""}" data-reset="${part}" title="${esc(text)} · klik for at nulstille" aria-label="${esc(text)}. Klik for at nulstille"><ha-icon icon="${icon}"></ha-icon></button>`;
  }
  render() {
    ...
    this.shadowRoot.innerHTML=`<style>
      ...
      .scene{height:238px;margin:14px -6px;display:grid;place-items:center;position:relative;isolation:isolate;overflow:hidden;border-radius:18px;background:radial-gradient(circle at 50% 45%,color-mix(in srgb,var(--state) 17%,transparent),transparent 38%),linear-gradient(160deg,#18242d,#0d1217 62%,#151a1e)}.floor{position:absolute;inset:-44px;background-image:linear-gradient(#ffffff0b 1px,transparent 1px),linear-gradient(90deg,#ffffff0b 1px,transparent 1px);background-size:42px 42px}.cleaning .floor,.returning .floor{animation:floorFlow 2.4s linear infinite}.glow{position:absolute;width:220px;height:110px;border-radius:50%;background:var(--state);filter:blur(44px);opacity:.15;transform:translateY(50px);z-index:-1}.robotWrap{position:relative;width:150px;height:150px;display:grid;place-items:center;z-index:3;filter:drop-shadow(0 18px 14px #0009)}.robot{z-index:2;width:124px;height:124px;border-radius:50%;background:linear-gradient(145deg,#3c4650,#10151a);border:3px solid #66727c;box-shadow:inset 0 2px 5px #ffffff2b;position:relative}.robot:before{content:"";position:absolute;inset:17px;border-radius:50%;border:2px solid #77818a;background:radial-gradient(circle at 45% 40%,#46515a,#1b2025 68%)}.lid{position:absolute;width:38px;height:28px;border-radius:10px;left:43px;top:26px;background:#0d1115;border:2px solid #56616a}.clean{position:absolute;width:19px;height:19px;border-radius:50%;left:52px;top:64px;background:var(--state);box-shadow:0 0 14px var(--state)}.particles{position:absolute;inset:0;z-index:2;pointer-events:none}.dust{position:absolute;left:50%;top:8%;width:5px;height:5px;border-radius:50%;background:#c7b99b;opacity:0}.cleaning .dust{animation:dustFlow 2.8s linear infinite}.dust:nth-child(2){margin-left:-54px;animation-delay:-.45s;animation-duration:3.2s}.dust:nth-child(3){margin-left:42px;animation-delay:-1.1s;animation-duration:2.45s}.dust:nth-child(4){margin-left:-28px;animation-delay:-1.65s;animation-duration:3.5s}.dust:nth-child(5){margin-left:65px;animation-delay:-2.1s;animation-duration:2.9s}.dust:nth-child(6){margin-left:14px;animation-delay:-2.55s;animation-duration:3.7s}.dock{z-index:1;position:absolute;left:calc(50% + 48px);top:calc(50% - 31px);width:50px;height:62px;border-radius:12px 12px 4px 4px;background:linear-gradient(#4d5962,#151a1f);border:2px solid #68757f;box-shadow:0 0 28px color-mix(in srgb,var(--state) 28%,transparent);transition:transform 1.15s cubic-bezier(.2,.8,.2,1),opacity .7s}.dock:before{content:"";position:absolute;width:10px;height:10px;border-radius:50%;background:var(--state);left:18px;top:12px;box-shadow:0 0 12px var(--state)}.dock:after{content:"";position:absolute;left:-12px;right:-12px;bottom:-8px;height:12px;border-radius:10px;background:#23292e;border:2px solid #59636c}.docked .dock,.charging .dock{transform:translateX(0);opacity:1}.cleaning .dock,.paused .dock,.idle .dock,.error .dock{transform:translateX(150px);opacity:.28}.returning .dock{animation:dockInvite 2.2s ease-in-out infinite}.returnPulse{position:absolute;left:calc(50% + 92px);top:calc(50% - 5px);width:10px;height:10px;border-radius:50%;background:var(--state);opacity:0}.returning .returnPulse{animation:returnSignal 1.6s ease-out infinite}.returning .returnPulse+.returnPulse{animation-delay:.5s}.charging .clean,.docked .clean{animation:pulse 2s ease-in-out infinite}.error .robot{animation:shake .5s linear infinite}.alerts{position:absolute;z-index:6;top:10px;left:10px;right:10px;display:flex;flex-wrap:wrap;gap:6px;pointer-events:none}.alert{display:flex;gap:6px;align-items:center;max-width:100%;padding:6px 9px;border-radius:999px;font-size:12px;line-height:1.2;background:color-mix(in srgb,var(--warning-color,#ff9800) 22%,#111d);border:1px solid color-mix(in srgb,var(--warning-color,#ff9800) 52%,transparent);box-shadow:0 5px 16px #0007;backdrop-filter:blur(5px)}.alert ha-icon{width:18px;height:18px;color:var(--warning-color,#ff9800);flex:0 0 auto}
      .controls{display:grid;grid-template-columns:repeat(auto-fit,minmax(72px,1fr));gap:9px}button{border:1px solid var(--divider-color);background:var(--secondary-background-color);color:var(--primary-text-color);min-height:58px;border-radius:12px;padding:8px 6px;display:grid;place-items:center;gap:4px;cursor:pointer;font:inherit;font-size:12px}button.primary{background:var(--accent);color:#fff;border-color:transparent}button:disabled{opacity:.55}.maintenanceIcons{position:absolute;z-index:7;left:10px;bottom:10px;display:flex;gap:7px}.serviceIcon{min-height:0;width:38px;height:38px;padding:0;border-radius:50%;color:var(--success-color,#43a047);background:#111d;border-color:color-mix(in srgb,currentColor 50%,transparent);box-shadow:0 5px 16px #0007;backdrop-filter:blur(5px)}.serviceIcon ha-icon{width:21px;height:21px}.serviceIcon:hover{transform:translateY(-1px);box-shadow:0 7px 18px #0009}.serviceIcon.warn{color:var(--warning-color,#ff9800);background:color-mix(in srgb,var(--warning-color,#ff9800) 18%,#111d)}.serviceIcon.overdue{color:var(--error-color,#db4437);background:color-mix(in srgb,var(--error-color,#db4437) 20%,#111d);animation:pulse 1.2s infinite}.serviceIcon.muted{color:var(--secondary-text-color,#888)}.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(105px,1fr));gap:8px;margin-top:14px;padding-top:14px;border-top:1px solid var(--divider-color)}.stat{background:var(--secondary-background-color);padding:9px 10px;border-radius:10px;display:grid;gap:3px;cursor:pointer}.stat span{font-size:11px;color:var(--secondary-text-color)}.stat b{font-size:14px}.missing{padding:20px}.spin{animation:rotate 1s linear infinite}
      @keyframes rotate{to{transform:rotate(360deg)}}@keyframes pulse{50%{opacity:.45}}@keyframes floorFlow{from{background-position:0 0,0 0}to{background-position:0 84px,0 84px}}@keyframes dustFlow{0%{transform:translate(-12px,-8px) scale(.65);opacity:0}18%{opacity:.72}78%{opacity:.5}100%{transform:translate(0,100px) scale(.15);opacity:0}}@keyframes dockInvite{0%,100%{transform:translateX(92px);opacity:.55}50%{transform:translateX(62px);opacity:1}}@keyframes returnSignal{0%{transform:translateX(0) scale(.45);opacity:0}25%{opacity:.8}100%{transform:translateX(-88px) scale(.15);opacity:0}}@keyframes shake{25%{transform:translateX(-4px)}75%{transform:translateX(4px)}}@media(max-width:420px){.card{padding:16px}.scene{height:210px}.controls{grid-template-columns:repeat(3,1fr)}}@media(prefers-reduced-motion:reduce){*{animation-duration:.001ms!important;animation-iteration-count:1!important}}
    ...
    <div class="scene ${esc(state)}"><div class="floor"></div><div class="glow"></div><div class="alerts">${binFull?`<div class="alert"><ha-icon icon="mdi:delete-alert"></ha-icon>Beholderen er fuld og bør tømmes</div>`:""}${!binPresent?`<div class="alert"><ha-icon icon="mdi:delete-off"></ha-icon>Beholderen er ikke monteret</div>`:""}</div>${maintenance?`<div class="maintenanceIcons">${maintenance}</div>`:""}<div class="particles"><i class="dust"></i><i class="dust"></i><i class="dust"></i><i class="dust"></i><i class="dust"></i><i class="dust"></i></div><div class="returnPulse"></div><div class="returnPulse"></div><div class="robotWrap"><div class="robot"><div class="lid"></div><div class="clean"></div></div></div><div class="dock"></div></div>
    <div class="controls">${controls}</div>${this._config.show_stats!==false&&stats?`<div class="stats">${stats}</div>`:""}</div></ha-card>`;
